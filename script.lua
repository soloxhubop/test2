local GITHUB_KEYS_URL = "https://raw.githubusercontent.com/soloxhubop/test/refs/heads/main/keys.txt"; local WEBHOOK_URL = "https://discord.com/api/webhooks/1479874056097632256/f95iwk7QqvFqVkwd_1_BMVeRr_3JqZjtGnlcCPgUAT2aOLSNoY9CcejnJV3CB8xEc1-S"
...
if isAdmin() then
    ...
    local adminStroke = Instance.new("UIStroke", adminBtn); adminStroke.Color = Color3.fromRGB(170,80,255); adminStroke.Thickness = 1.5
    adminBtn.MouseButton1Click:Connect(function()
        ...
        for i, t in ipairs({{"1 Giorno","1D",Color3.fromRGB(180,60,60)},{"2 Settimane","2W",Color3.fromRGB(60,100,200)},{"Lifetime","LT",Color3.fromRGB(120,40,240)}}) do
            ...
            b.Font=Enum.Font.GothamBold; b.TextSize=13; b.AutoButtonColor=false; b.ZIndex=51; Instance.new("UICorner", b).CornerRadius = UDim.new(0, 8)
            b.MouseButton1Click:Connect(function()
                local expiry = getExpiryDate(t[2]); local newKey = generateKey(t[2])
                sendKeyToWebhook(newKey.."|"..expiry); pStatus.Text="Inviata su Discord!"; b.Text="Inviata!"
...
dotFrame.Size = UDim2.new(0,150,0,16); dotFrame.Position = UDim2.new(0.5,-75,0,172); dotFrame.BackgroundTransparency = 1; dotFrame.ZIndex = 12
local dotDot = Instance.new("Frame", dotFrame)
dotDot.Size = UDim2.new(0,9,0,9); dotDot.Position = UDim2.new(0,0,0.5,-4)
dotDot.BackgroundColor3 = Color3.fromRGB(80,220,80); dotDot.BorderSizePixel = 0; dotDot.ZIndex = 12
Instance.new("UICorner", dotDot).CornerRadius = UDim.new(0.5, 0)
...
local function makeSocialBtn(text, idx)
    ...
    local bs = Instance.new("UIStroke", btn); bs.Color = Color3.fromRGB(100,50,180); bs.Thickness = 1.5
    return btn
end
local discordBtn = makeSocialBtn("Discord", 0); makeSocialBtn("Twitter", 1); makeSocialBtn("YouTube", 2)
discordBtn.MouseButton1Click:Connect(function()
    pcall(function() setclipboard(DISCORD_LINK) end)
    discordBtn.Text = "Copiato!"; task.delay(2, function() discordBtn.Text = "Discord" end)
end)
local function launchMainScript()
    ...
    local function AddOutline(frame)
        local s = Instance.new("UIStroke", frame); s.Color = Color3.fromRGB(255,255,255); s.Thickness = 1; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; s.Transparency = 0.6
    end
    ...
    local SETTINGS = {
        ...
        SPEED_ENABLED=true, LOCK_ENABLED=false, RADIUS=8,
        STEAL_DURATION=0.2, TARGET_SPEED=58, LOCK_SPEED=59,
        JUMP_FORCE=58, UNWALK=false, SPIN_ENABLED=false, SPIN_SPEED=100,
        STEAL_SPEED=29.40,
    ...
    local floatPart, floatHeight = nil, 0; local isStealing = false; local currentTween = nil; local tpSide = nil
    local tpAutoEnabled = false; local autoSwingActive = false
    local DEFAULT_POSITIONS = {
        Save      = UDim2.new(0,   8, 0,   8),
        Drop      = UDim2.new(0,   8, 0,  44),
        AutoGrab  = UDim2.new(0,   8, 0,  92),
        Lock      = UDim2.new(0,   8, 0, 140),
        Float     = UDim2.new(0,   8, 0, 188),
        Taunt     = UDim2.new(0,   8, 0, 236),
        SilentTP  = UDim2.new(0.5,-55, 0,   8),
        MenuBtn   = UDim2.new(1,-118, 0,   8),
        AutoLeft  = UDim2.new(1,-118, 0,  56),
        AutoRight = UDim2.new(1,-118, 0, 104),
    }; local savedPositions = {}; local L_POS_1      = Vector3.new(-476.48, -6.28, 92.73); local L_POS_END    = Vector3.new(-483.12, -4.95, 94.80)
    ...
    local function saveMenuConfig()
        ...
        pcall(function() writefile(MENU_SAVE_KEY..".json", HttpService:JSONEncode(d)) end)
    end
                    ...
                    if tpAutoEnabled and tpSide then
                        task.spawn(function()
                            ...
                            elseif tpSide == "RIGHT" then
                                r2.CFrame = CFrame.new(TP_RIGHT_1); task.wait(0.03); r2.CFrame = CFrame.new(TP_RIGHT_2)
                            end
                        end)
                    end
                end
    ...
    UserInputService.JumpRequest:Connect(function()
        ...
        hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, SETTINGS.JUMP_FORCE, hrp.AssemblyLinearVelocity.Z)
    end)
    ...
    local function startAutoSwing()
        ...
        task.spawn(function()
            while autoSwingActive and SETTINGS.LOCK_ENABLED do
                equipBat(); silentSwing(); silentSwing(); task.wait(0.18)
            end
            ...
            if p ~= LocalPlayer and p.Character then
                ...
                if eh and hum and hum.Health > 0 then
                    ...
                    if d < nearestDist then
                        nearestDist=d; nearest=eh; nearestTorso = p.Character:FindFirstChild("UpperTorso") or p.Character:FindFirstChild("Torso") or eh
                    end
                end
            end
        end
    ...
    local function applySpin()
        ...
        local s = Instance.new("BodyAngularVelocity"); s.Name="BlaalSpin"; s.Parent=root; s.MaxTorque=Vector3.new(0,math.huge,0); s.P=1200; s.AngularVelocity=Vector3.new(0,SETTINGS.SPIN_SPEED,0)
    end
    ...
    LocalPlayer.CharacterAdded:Connect(function(char)
        ...
        if not LocalPlayer:FindFirstChild("BS_DUEL_USER") then
            local t2=Instance.new("StringValue",LocalPlayer); t2.Name="BS_DUEL_USER"; t2.Value="using bs duel"
        end
    end)
    ...
    local function getPromptPos(prompt)
        ...
        else local pt=p:FindFirstChildWhichIsA("BasePart",true); return pt and pt.Position end
    end
    local function findNearestStealPrompt()
        ...
        local searchRoot = workspace:FindFirstChild("Plots") or workspace; local myPos = root.Position; local nearestPrompt, nearestDist = nil, math.huge
        for _, obj in ipairs(searchRoot:GetDescendants()) do
            if obj:IsA("ProximityPrompt") and obj.Enabled and obj.ActionText=="Steal" then
                local pos = getPromptPos(obj)
                if pos then
                    local dist = (myPos-pos).Magnitude
                    if dist<=obj.MaxActivationDistance and dist<nearestDist then nearestPrompt=obj; nearestDist=dist end
                end
            end
        end
        ...
        while SETTINGS.ENABLED do
            ...
            if hum then
                ...
                if prompt then
                    ...
                    pcall(function() fireproximityprompt(prompt,99999) end)
                    pcall(function() prompt:InputHoldBegin() end)
                    task.wait(0.02)
                    pcall(function() prompt:InputHoldEnd() end)
                    task.wait(0.02); ProgressBarFill.Size=UDim2.new(0,0,1,0); isStealing=false
                end
            end
            task.wait(0.05)
        end
    ...
    local function SetHUDState(btn, label, on)
        ...
        if on then
            ...
            local s=btn:FindFirstChildOfClass("UIStroke")
            if s then s.Color=Color3.fromRGB(255,255,255); s.Thickness=1; s.Transparency=0.6 end
        end
    end
    local function MakeHUDButton(name, label, onToggle)
        ...
        return btn, function(state) active=state; SetHUDState(btn,label,state) end
    end
    ...
    local function buildSpeedPanel(anchorBtn)
        ...
        Instance.new("UICorner", closeG).CornerRadius = UDim.new(0.5, 0)
        closeG.MouseButton1Click:Connect(function() panelGui:Destroy(); speedPanel=nil; speedPanelOpen=false end)
        ...
        local function MakeSpeedRow(labelTxt, getVal, setVal, yPos)
            ...
            local bs2 = Instance.new("UIStroke", box); bs2.Color = Color3.fromRGB(255,255,255); bs2.Thickness = 1; bs2.Transparency = 0.6
    ...
    local gearLeft = Instance.new("TextButton", AutoLeftHUD); gearLeft.Size = UDim2.new(0,20,0,20); gearLeft.Position = UDim2.new(1,-22,0,2); gearLeft.BackgroundColor3 = Color3.fromRGB(0,0,0); gearLeft.BackgroundTransparency = 0.4; gearLeft.BorderSizePixel = 0; gearLeft.Text = "Ã¢ÂšÂ™"; gearLeft.TextColor3 = Color3.fromRGB(255,255,255)
    ...
    gearRight.Text = "Ã¢ÂšÂ™"; gearRight.TextColor3 = Color3.fromRGB(255,255,255); gearRight.Font = Enum.Font.GothamBold; gearRight.TextSize = 14; gearRight.AutoButtonColor = false; gearRight.ZIndex = 7; Instance.new("UICorner", gearRight).CornerRadius = UDim.new(0.5, 0)
        ...
        if on then
            task.spawn(function()
                while SETTINGS.TAUNT_ON do
                    pcall(function() local TCS=game:GetService("TextChatService"); local ch=TCS.TextChannels:FindFirstChild("RBXGeneral"); if ch then ch:SendAsync("meloska duel") end end)
                    task.wait(0.5)
                end
            end)
        end
    ...
    local DropHUD, setDropState = MakeHUDButton("Drop","DROP",function(on)
        if on then
            task.spawn(function()
                ...
                SETTINGS.DROP_ON=false; setDropState(false)
            end)
        end
        SETTINGS.DROP_ON=on
    end)
    local function updateTpButton() end
    local silentTpEnabled = false
    local silentTpWasRagdoll = false
    ...
    local function silentTpGetSide()
        ...
        for _, pl in ipairs(plots:GetChildren()) do
            ...
            if s and s:FindFirstChild("YourBase") and s.YourBase.Enabled and b then
                ...
                if t then
                    local pos = t.Position; local dl = (pos - STP_LEFT[1]).Magnitude; local dr = (pos - STP_RIGHT[1]).Magnitude
    ...
    local function doSilentTp()
        local side = silentTpGetSide(); if not side then return end
        local char = LocalPlayer.Character; if not char then return end
        char:PivotTo(CFrame.new(side=="left" and STP_LEFT[1] or STP_RIGHT[1])); task.wait(0.1); char:PivotTo(CFrame.new(side=="left" and STP_LEFT[2] or STP_RIGHT[2]))
    end
    if LocalPlayer.Character then
        local h = LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
        if h then silentTpHum = h end
    end
    LocalPlayer.CharacterAdded:Connect(function(char)
        char:WaitForChild("Humanoid"); silentTpHum = char:FindFirstChildOfClass("Humanoid")
        silentTpWasRagdoll = false
    end)
    ...
    Instance.new("UICorner",SilentTPHUD).CornerRadius=UDim.new(0,8); AddOutline(SilentTPHUD); allButtons["SilentTP"]=SilentTPHUD
    MakeDraggable(SilentTPHUD, function()
        ...
        SilentTPHUD.BackgroundColor3=Color3.fromRGB(0,0,0); SilentTPHUD.BackgroundTransparency=silentTpEnabled and 0.2 or 0.45
    end)
    ...
    local function buildMenuPanel()
        ...
        closeBtn2.MouseButton1Click:Connect(function()
            menuOpen=false; panelGui:Destroy(); menuPanel=nil; MenuBtn.Text="MENU"; MenuBtn.BackgroundColor3=Color3.fromRGB(0,0,0); MenuBtn.BackgroundTransparency=0.45; MenuBtn.TextColor3=Color3.fromRGB(255,255,255)
        end)
        ...
        menuSaveBtn.MouseButton1Click:Connect(function()
            ...
            task.delay(1.5, function() menuSaveBtn.Text="SAVE MENU CONFIG"; menuSaveBtn.TextColor3=Color3.fromRGB(255,255,255); menuSaveBtn.BackgroundColor3=Color3.fromRGB(22,22,22) end)
        end)
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if SETTINGS.LOCK_ENABLED then
            ...
            if target and torso then
                ...
                if fullDir.Magnitude > 0.8 then
                    ...
                    hrp.AssemblyLinearVelocity = Vector3.new(target.AssemblyLinearVelocity.X, hrp.AssemblyLinearVelocity.Y, target.AssemblyLinearVelocity.Z)
                end
            end
        ...
        if SETTINGS.FLOAT then
            if not floatPart then
                ...
                floatPart = Instance.new("BodyPosition", hrp)
                floatPart.Name = "MelFloatBP"
                floatPart.MaxForce = Vector3.new(0, math.huge, 0)
                floatPart.D = 1000; floatPart.P = 10000
                floatPart.Position = Vector3.new(hrp.Position.X, floatHeight, hrp.Position.Z)
            else
                floatPart.Position = Vector3.new(hrp.Position.X, floatHeight, hrp.Position.Z)
            end
        ...
        if moveActive then
            ...
            if dist2 < 0.5 then
                if moveActive=="Left" then
                    ...
                    else SETTINGS.AUTORIGHT=false; setAutoRightState(false); RightPhase=1 end
                end
            else
                local dir = (target2 - hrp.Position).Unit; hrp.AssemblyLinearVelocity = Vector3.new(dir.X*speed, hrp.AssemblyLinearVelocity.Y, dir.Z*speed)
            end
        end
    end)
end
local function tryRedeem(inputKey)
    ...
    if valid then
        ...
        redeemBtn.Text="> Redeem"; redeemBtn.BackgroundColor3=Color3.fromRGB(120,40,240);statusLbl.Text="X "..(info or "Key non valida!"); statusLbl.TextColor3=Color3.fromRGB(255,80,80)
    end
end
...
if savedKey and savedKey ~= "" then
    local cleanSaved = (savedKey:match("^([^|]+)") or savedKey):match("^%s*(.-)%s*$"); keyBox.Text = cleanSaved
    task.spawn(function() task.wait(0.5); tryRedeem(cleanSaved) end)
end
