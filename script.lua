...
local function launchMainScript()
        ...
        while SETTINGS.ENABLED do
            ...
            if hum and hum.WalkSpeed > 29 then
                ...
                if prompt then
                    ...
                    currentTween:Play(); task.wait(0.1)
    ...
    local DropHUD, setDropState = MakeHUDButton("Drop","DROP",function(on)
        if on then
            task.spawn(function()
                local char=LocalPlayer.Character
                if not char then SETTINGS.DROP_ON=false; setDropState(false); return end
                local dropped = false
                -- Cerca ProximityPrompt "Drop" su tutto il character (brainrot tenuto in mano)
                for _, obj in ipairs(char:GetDescendants()) do
                    if obj:IsA("ProximityPrompt") and (obj.ActionText=="Drop" or obj.ActionText=="drop") then
                        pcall(function() fireproximityprompt(obj) end)
                        pcall(function() obj:InputHoldBegin(); task.wait(0.05); obj:InputHoldEnd() end)
                        dropped = true; break
                    end
                end
                -- Cerca anche nel workspace vicino al player (brainrot potrebbe essere li)
                if not dropped then
                ...
                -- Fallback: premi Q/G (tasto drop default Roblox) via VirtualInputManager
                if not dropped then
                    pcall(function()
                ...
                task.wait(0.2); SETTINGS.DROP_ON=false; setDropState(false)
            end)
        end
        SETTINGS.DROP_ON=on
    end)
